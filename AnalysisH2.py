    ...
    for sheet in data:
        ...
        for row in sheet:
           ...
           # print(len(plant))
            ...
            powerplant = PowerPlantH2.PowerPlant(plant)
...
def indepthAnalysis (data,file):
    ...
    for sheet in data:
       ...
       # print(len(plant))
        ...
        powerplant = PowerPlantH2.PowerPlant(sheet)

        capex = powerplant.capex_total
        opex = powerplant.opex_total_musd


        lcoe = KPIs.LCOE_simple((capex-powerplant.H2_capex),(opex-powerplant.H2_opex),sum(combined_generaton[0])/1000)
        lcoh = KPIH2.lcoh(powerplant.H2_capex,powerplant.H2_opex,sum(combined_generaton[4]))
        #powerplant.lcoe = lcoe


        cf = KPIH2.cf((powerplant.csp_cap+powerplant.pv_cap/1000),sum(combined_generaton[0])) #capacity factor
        #powerplant.capacityFactor = cf

        peakProd = KPIH2.peak_prod(combined_generaton[0]) # ratio of generation in peak hours
        ...
        ppa = KPIH2.PPA(capex,opex,combined_generaton[0],8760*peakProd,combined_generaton)
        print('PPA: ', ppa, 'USD/kWh')
        npv = KPIH2.NPV(capex, opex, ppa, combined_generaton[0], 8760 * peakProd,combined_generaton)  # imputs needed for PPA!!!
        print('NPV: ', npv[0])
        ...
        sheet.append(combined_generaton[4]) # kh H2
        #sheet.append(npv[1])

        allSheets.append(sheet)
        print('H2 CAPEX: ', powerplant.H2_capex)
        print('H2 OPEX: ', powerplant.H2_opex)
...
def simpleStrategy(csp_cap,csp,pv,inverterCap,inverterEff,cut_good,cut_bad,TES_cap,PVselfCon,TES_loss):
    ...
    for hours in range(len(csp)):
        ...
        if hoursGood[hours]:
            ...
            outputSimple = SimpleGoodDay(csp_cap,csp,pv,cut_good,combined,pvProd,inverterCap,inverterEff,cspProd,TES_SOC,TES_cap,TES_dis,hours,t,PVselfCon,TES_loss,H_2)
            ...
            outputSimple = SimpleGoodDay(csp_cap, csp, pv, cut_bad, combined, pvProd, inverterCap, inverterEff,
                                         cspProd, TES_SOC, TES_cap, TES_dis, hours, t, PVselfCon, TES_loss,H_2)
            #TES_SOC[hours] = outputSimple.TES_SOC
            #print(TES_dis)
    ...
    return [combined, pvProd,TES_dis,TES_SOC,H_2]

def SimpleBadDay(csp_cap,csp,pv,cut,combined,pvProd,inverterCap,inverterEff,cspProd,TES_SOC,TES_cap,TES_dis,hour,t,PVselfCon,TES_loss,H_2):
    SimpleGoodDay(csp_cap, csp, pv, cut, combined, pvProd,inverterCap,inverterEff, cspProd, TES_SOC, TES_cap, TES_dis, hour, t,PVselfCon,TES_loss,H_2)


def SimpleGoodDay(csp_cap,csp,pv,cut,combined,pvProd,inverterCap,inverterEff,cspProd,TES_SOC,TES_cap,TES_dis,hour,t,PVselfCon,TES_loss,H_2):

    #pvSurplus = dispatchPV(cut,pv,pvProd,hour,t)
    pvSurplus = dispatchPVDC(cut,pv,inverterCap,inverterEff,pvProd,hour,t,PVselfCon,H_2)

    dispatchStorage(csp_cap,csp,cspProd,pvProd,TES_SOC,TES_cap,TES_dis,hour,t,TES_loss)
...
def dispatchPVDC(cut,pv,inverterCap,inverterEff, pvProd,hour,t,selfCon,H_2):
    ...
    if pv[hour] >E_dc_max:
        ...
        H_2[hour] = (pvSurplus*h2_eff)/h2Const
    else:

        pvProd[hour] = max(pv[hour]*inverterEff - selfCon,0)
        pvSurplus = max(0,pv[hour]*inverterEff-selfCon-pvProd[hour])

    return pvSurplus

def dispatchStorage(csp_cap,csp,cspProd,pvProd,TES_SOC,TES_cap,TES_dis,hour,t,TES_losses):
    ...
    a = hour
    startUpFactor = 0.5
    csp_max = 105000 - pvProd[hour] # maximum dispatch of csp into grid
